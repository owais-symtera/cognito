"""
Initialize pharmaceutical categories in SQLite database.
This script creates a SQLite database and populates it with the 17 pharmaceutical intelligence categories.
"""
...
DB_FILE = "cognitoai_categories.db"
...
CATEGORIES = [
    # Phase 1: Data Collection Categories (1-10)
    {
        ...
        "weight": 1.0,
        "source_priorities": json.dumps(["paid_apis", "government", "industry", "peer_reviewed"])
    },
    {
        "id": 2,
        ...
        "weight": 1.0,
        "source_priorities": json.dumps(["paid_apis", "industry", "peer_reviewed", "company"])
    },
    {
        "id": 3,
        ...
        "weight": 1.2,
        "source_priorities": json.dumps(["government", "paid_apis", "peer_reviewed"])
    },
    {
        "id": 4,
        ...
        "weight": 0.9,
        "source_priorities": json.dumps(["paid_apis", "industry", "peer_reviewed", "news"])
    },
    {
        "id": 5,
        ...
        "weight": 0.8,
        "source_priorities": json.dumps(["government", "paid_apis", "company"])
    },
    {
        "id": 6,
        ...
        "weight": 0.7,
        "source_priorities": json.dumps(["government", "company", "peer_reviewed", "paid_apis"])
    },
    {
        "id": 7,
        ...
        "weight": 0.6,
        "source_priorities": json.dumps(["paid_apis", "peer_reviewed", "government"])
    },
    {
        "id": 8,
        ...
        "weight": 0.9,
        "source_priorities": json.dumps(["government", "peer_reviewed", "paid_apis"])
    },
    {
        "id": 9,
        "key": "dosage_forms",
        ...
        "weight": 0.8,
        "source_priorities": json.dumps(["government", "peer_reviewed", "company"])
    },
    {
        "id": 10,
        ...
        "weight": 1.1,
        "source_priorities": json.dumps(["government", "peer_reviewed", "paid_apis", "industry"])
    },
...
def init_sqlite_categories():
    ...
    if count > 0:
        print(f"Categories already exist in database ({count} found). Skipping initialization.")
        conn.close()
        return
    ...
    for cat in CATEGORIES:
        ...
        ''', (
            ...
            cat["prompt_template"],
            cat["weight"],
            cat["source_priorities"],
            cat.get("requires_phase1", 0)
        ))

    conn.commit()
    print(f"Successfully initialized {len(CATEGORIES)} pharmaceutical categories in SQLite database.")

    # Verify insertion
    cursor.execute("SELECT id, name, phase, enabled FROM pharmaceutical_categories ORDER BY id")
