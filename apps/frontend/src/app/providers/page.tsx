...
export default function ProvidersPage() {
  ...
  // Test provider mutation with feedback
  const testMutation = useMutation({
    mutationFn: async (providerId: string) => {
      ...
      const data = await response.json()
      if (!response.ok) {
        throw new Error(data.detail || 'Failed to test provider')
      }
      return data
    },
    onSuccess: (data, providerId) => {
      ...
      setTimeout(() => {
        setTestResults(prev => {
          const updated = { ...prev }
          if (updated[providerId]?.timestamp === Date.now()) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {providers.map((provider) => {
            const temps = provider.temperatures || []
            const isTestingThis = testMutation.isPending && testMutation.variables === provider.id
            return (
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">
                      {provider.supports_temperature === false ? '1 call' : `${temps.filter(t => t.enabled).length}/${temps.length} temps`}
                    </Badge>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Button
                      ...
                      className="w-full"
                      onClick={() => {
                        // Clear previous test result for this provider
                        setTestResults(prev => {
                          const updated = { ...prev }
                      ...
                      disabled={!provider.enabled || isTestingThis}
                    >
                      {isTestingThis ? (
                        <>
            ...
            {editProvider && (
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-3 p-3 border rounded-lg bg-muted/30">
                  <div className="flex items-center space-x-2">
                    <Switch
                      checked={editProvider.supports_temperature !== false}
                      onCheckedChange={(supports_temperature) => setEditProvider({ ...editProvider, supports_temperature })}
                    />
                    <div className="flex-1">
                      <Label className="font-semibold">Temperature Support</Label>
                      <p className="text-xs text-muted-foreground mt-1">
                        {editProvider.supports_temperature !== false
                          ? 'API will be called with each enabled temperature setting'
                          : 'API will be called only once (temperature parameter not supported)'}
                      </p>
                    </div>
                  </div>
                  {editProvider.supports_temperature === false && (
                    <Alert>
                      ...
                      <AlertDescription className="text-xs">
                        When disabled, the API will be called only once regardless of temperature configurations.
                        This is useful for models that don't support the temperature parameter (e.g., o1-preview, search models).
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
                ...
                onClick={async () => {
                  if (editProvider) {
                    ...
                    await updateMutation.mutateAsync({
                      ...
                      supports_temperature: editProvider.supports_temperature,
                      temperatures: editProvider.temperatures
                    })
